/****************************************************************************
 *
 * $Id: v2u_dec.c 20767 2013-02-19 20:32:52Z pzeldin $
 *
 ...
 */
static int save_frames(V2U_DRIVER_HANDLE v2u, FILE* out, const V2UCompressParams* cp, const V2U_VideoMode *vm)
{
    int result = 0;
    int i = 0;
    int totalsize = 0;
    ...
    int bpp = V2UPALETTE_2_BPP(palette);
    struct ioctl_grabframe2 gf;

    /* Allocate enough memory for full frame + overhead */
    memset(&gf, 0, sizeof(gf));
    gf.pixbuflen = vm->width*vm->height*(bpp/8) * 11/10;
    gf.pixbuf = malloc(gf.pixbuflen);

    /* Make sure that the first frame is a key frame */
    gf.palette = palette | V2U_GRABFRAME_KEYFRAME_FLAG;
    if (gf.pixbuf) {
        for( i=0; i<cp->count; i++ ) {
            /* Reset the crop rectangle to always grab full frame */
            memset(&gf.crop, 0, sizeof(gf.crop) );

            /* Grab the next frame */
            if (!v2u_ioctl(v2u, IOCTL_VGA2USB_GRABFRAME2, &gf, sizeof(gf)) ||
                gf.imagelen <= 0) {
                printf("VGA2USB ioctl error\n");
                result = STATUS_GRABERR;
                break;
            }

            if (gf.imagelen >= gf.pixbuflen) {
                /* Buffer is not large enough, re-grab with V2U_GRABFRAME_KEYFRAME_FLAG */
                gf.palette |= V2U_GRABFRAME_KEYFRAME_FLAG;
                if (!v2u_ioctl(v2u, IOCTL_VGA2USB_GRABFRAME2, &gf,
                               sizeof(gf)) || gf.imagelen <= 0) {
                    printf("VGA2USB ioctl error\n");
                    result = STATUS_GRABERR;
                    break;
                }
            ...
            if (!fwrite32(out, gf.imagelen) ||
                fwrite(gf.pixbuf, gf.imagelen, 1, out) != 1) {
            ...
            gf.palette &= ~V2U_GRABFRAME_KEYFRAME_FLAG;
            totalsize += gf.imagelen + sizeof(gf.imagelen);
        }
        free(gf.pixbuf);
    } else {
        result = STATUS_FAIL;
    ...
    printf("Wrote %d frames, %d bytes, %d ms, %.2f fps, starttime %d\n", i, totalsize,
...
int main(int argc, char * argv[])
{
    ...
    if (!strcmp("x", argv[1])) {
        ...
        if (result == STATUS_OK) {
            V2U_DRIVER_HANDLE d = v2u_open_driver();
            if (d) {

                /* Check if hardware compression is supported */
                V2U_Property p;
                p.key = V2UKey_HardwareCompression;
                if (v2u_ioctl(d, IOCTL_VGA2USB_GET_PROPERTY, &p, sizeof(p))) {
                    const char* pn = v2u_product_name(v2u_get_product_type(d));
                    if (p.value.boolean) {
                        V2U_VideoMode vm;
                        ...
                        if (cp.level >= 0) {
                            ...
                            v2u_set_property(d, filter.key, &filter.value);
                        }

                        /* Detect video mode */
                        if (v2u_detect_videomode(d,&vm) &&
                            vm.width && vm.height) {
                            FILE* out;
                            ...
                            if (out) {
                                printf("Writing %s\n", cp.fname);
                                result = save_frames(d, out, &cp, &vm);
                                fclose(out);
                            } else {
                                printf("Failed to open file %s\n", cp.fname);
                                result = STATUS_IOERR;
