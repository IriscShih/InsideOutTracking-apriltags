/****************************************************************************
 *
 * $Id: v2u_util.c 22137 2013-05-14 11:08:56Z monich $
 *
 ...
 *
 * Extended EDID looks a bit different, e.g.
 *
 * EDID BYTES:
 * 0x     00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F
 *       ------------------------------------------------
 * 0000 | 00 FF FF FF FF FF FF 00 4C 2D C2 07 01 00 00 00
 * 0010 | 2E 14 01 03 80 66 39 78 0A EE 91 A3 54 4C 99 26
 * 0020 | 0F 50 54 BD EF 80 71 4F 81 00 81 40 81 80 95 00
 * 0030 | 95 0F B3 00 A9 40 02 3A 80 18 71 38 2D 40 58 2C
 * 0040 | 45 00 A0 5A 00 00 00 1E 66 21 50 B0 51 00 1B 30
 * 0050 | 40 70 36 00 A0 5A 00 00 00 1E 00 00 00 FD 00 18
 * 0060 | 4B 1A 51 17 00 0A 20 20 20 20 20 20 00 00 00 FC
 * 0070 | 00 53 41 4D 53 55 4E 47 0A 20 20 20 20 20 01 47
 * 0080 | 02 03 2B F1 46 90 04 05 03 20 22 23 09 07 07 83
 * 0090 | 01 00 00 E2 00 0F E3 05 03 01 70 03 0C 00 20 00
 * 00A0 | B8 2D 20 D0 06 01 40 00 37 20 50 01 1D 00 72 51
 * 00B0 | D0 1E 20 6E 28 55 00 A0 5A 00 00 00 1E 01 1D 80
 * 00C0 | 18 71 1C 16 20 58 2C 25 00 A0 5A 00 00 00 9E 8C
 * 00D0 | 0A D0 8A 20 E0 2D 10 10 3E 96 00 A0 5A 00 00 00
 * 00E0 | 18 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
 * 00F0 | 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 EF
 */

#define EDID_BUFSIZE         (60)
#define EDID_DIGITS_PER_LINE (16)
#define EDID_HEADER_LINES    (3)

typedef struct edid_format {
    V2U_STR* header;
    V2U_STR* prefix;
    int num_lines;
 ...
 */
static int v2u_edid_parse(FILE* in, V2U_UINT8* edid,
    const EDID_FORMAT* formats, int nformats)
{
    int i;
    size_t len;
    char line[EDID_BUFSIZE];
    const EDID_FORMAT* format = NULL;
    ...
    for (i=0; i<EDID_HEADER_LINES; i++) {
        ...
        if (format) {
            ...
            if (strcmp(format->header[i], line)) {
                return 0;
            }
        } else {
            /* If format is unknown, then the line should match either one of
             * the known formats or none of them. Anything else is considered
             * a parsing error */
            int k, match=0;
            for (k=0; k<nformats; k++) {
                if (!strcmp(formats[k].header[i], line)) {
                    match++;
                    format = formats + k;
                }
            }
            if (match == 1) {
                /* Single match means the we have selected the format */
            } else if (match == nformats) {
                /* All formats match, keep looking */
                format = NULL;
            } else {
    ...
    /* By now we should know the format */
    if (!format) {
        return 0;
    }

    /* Parse the data lines */
    for (i=0; i<format->num_lines; i++) {
        int k;
        const char* ptr;
        const char* prefix = format->prefix[i];
        ...
        if (len != format->line_len || strncmp(prefix, line, prefix_len)) {
            return 0;
        }
    ...
    return EDID_DIGITS_PER_LINE * format->num_lines;
}

/*
 * EDID format descriptors
 */

static V2U_STR edid_header_128[EDID_HEADER_LINES] = {
    "EDID BYTES:",
    "0x   00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F",
    "    ------------------------------------------------"
};

static V2U_STR edid_prefix_128[] = {
    "00 |","10 |","20 |","30 |","40 |","50 |","60 |","70 |",
};

static const char* edid_header_256[EDID_HEADER_LINES] = {
    "EDID BYTES:",
    "0x     00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F",
    "      ------------------------------------------------"
};

static V2U_STR edid_prefix_256[] = {
    "0000 |","0010 |","0020 |","0030 |","0040 |","0050 |","0060 |","0070 |",
    "0080 |","0090 |","00A0 |","00B0 |","00C0 |","00D0 |","00E0 |","00F0 |",
};

/*This array should be sorted by maximum EDID size */
#define N_(a) (sizeof(a)/sizeof((a)[0]))
static const EDID_FORMAT edid_formats [] = {
    {
        edid_header_128,
        edid_prefix_128,
        N_(edid_prefix_128),
...
/**
 * Reads text EDID from the stream. The text EDID must be in the
 * Phoenix EDID Designer format. Only handles base EDID format.
 * Buffer is assumed to be 128 bytes long.
 */
V2U_BOOL v2u_edid_read(FILE* in, V2U_UINT8 edid[/*V2U_EDID_SIZE*/])
{
    return v2u_edid_parse(in, edid, edid_formats, 1) != 0;
}

/**
 * Reads text EDID from the stream. The text EDID must be in the
 * Phoenix EDID Designer format. Handles both 128 and 256 byte EDIDs.
 * Buffer is assumed to be 128 bytes long. Returns EDID size (128 or 256)
 * or zero on failure.
 */
int v2u_edid_read_ext(FILE* in, V2U_UINT8 edid[/*V2U_EEDID_SIZE*/])
{
    return v2u_edid_parse(in, edid, edid_formats, N_(edid_formats));
}

/**
 * Writes EDID to the stream in the text form (Phoenix EDID Designer format).
 */
V2U_BOOL v2u_edid_write_text2(V2U_OUT_STREAM* out, V2U_TXT_STREAM_WRITER writer,
    const V2U_UINT8* edid, int len)
{
    /* Select format */
    const EDID_FORMAT* format = NULL;
    int i;
    for (i=0; i<N_(edid_formats); i++) {
        if (EDID_DIGITS_PER_LINE*edid_formats[i].num_lines >= len) {
            format = edid_formats + i;
            break;
        }
    }
    if (format) {
        int off = 0;
        for (i=0; i<EDID_HEADER_LINES; i++) {
            if (writer(format->header[i], out) == EOF ||
                writer("\n", out) == EOF) {
                return V2U_FALSE;
            }
        }
        for (i=0; i<format->num_lines; i++) {
            char line[EDID_BUFSIZE];
            int k;
            if (writer(format->prefix[i], out) == EOF) {
                return V2U_FALSE;
            }
            for (k=0; k<EDID_DIGITS_PER_LINE; k++) {
                if (sprintf(line+k*3, " %02X", (off<len)?edid[off]:0xff) != 3) {
                    return V2U_FALSE;
                }
                off++;
            }
            if (writer(line, out) == EOF) {
                return V2U_FALSE;
            }
            if (writer("\n", out) == EOF) {
                return V2U_FALSE;
...
V2U_BOOL v2u_edid_write_text(V2U_OUT_STREAM* out, V2U_TXT_STREAM_WRITER writer,
    const V2U_UINT8 edid[/*V2U_EDID_SIZE*/])
{
    return v2u_edid_write_text2(out, writer, edid, V2U_EDID_SIZE);
}

V2U_BOOL v2u_edid_write(FILE* out, const V2U_UINT8 edid[/*V2U_EDID_SIZE*/])
{
    return v2u_edid_write_ext(out, edid, V2U_EDID_SIZE);
}

struct v2u_out_stream_t {
    FILE* f;
};

static int text_file_write(const char* txt, V2U_OUT_STREAM* stream)
{
    return fputs(txt, stream->f);
}

V2U_BOOL v2u_edid_write_ext(FILE* out, const V2U_UINT8* edid, int len)
{
    V2U_OUT_STREAM stream;
    stream.f = out;
    return v2u_edid_write_text2(&stream, text_file_write, edid, len);
}
