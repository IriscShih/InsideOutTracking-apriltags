/****************************************************************************
 *
 * $Id: v2u_sys.c 21352 2013-03-23 10:42:37Z monich $
 *
...
/**
 * Opens named device and returns v2u_lib handle, NULL on failure.
 */
V2U_HANDLE v2u_open_driver_device(const char* dev)
{
    HANDLE hDriver = CreateFileA(dev, GENERIC_READ | GENERIC_WRITE, 0, NULL,
        OPEN_EXISTING, 0, NULL);
    if (hDriver != INVALID_HANDLE_VALUE) {
        V2U_HANDLE handle = malloc(sizeof(V2U_DRIVER));
        if (handle) {
...
V2U_HANDLE v2u_open_driver_idx(int idx)
{
    ...
    return v2u_open_driver_device(name);
}
 ...
 * zero on failure.
 */
V2U_BOOL v2u_ioctl(V2U_HANDLE h, unsigned long code, void * buf,
    unsigned int size)
{
    if (h) {
...
V2U_TIME v2u_time()
{
    ...
    now -= 11644473600000i32;   /* Jan 1st 1601 to Jan 1st 1970 */
    return now;
}
 ...
 * zero on failure.
 */
V2U_BOOL v2u_ioctl(V2U_HANDLE handle, unsigned long code, void * buf,
                   unsigned int size)
{
    if (handle) {
...
V2U_HANDLE v2u_open_driver_sn(const char* sn)
{
    if (sn) {
        int i;
        /* If there are multiple grabbers on a single board, one can
         * use "XXXXXXX.id" syntax, for example "D2P12345.video" */
        size_t sn_len = strlen(sn);
        const char* sep = strchr(sn, '.');
        const char* gid;
        if (sep) {
            gid = sep + 1;
            sn_len = sep - sn;
        ...
        for (i=MAX_VGA2USB_DEVICE_COUNT-1; i>=0; i--) {
            V2U_HANDLE h = v2u_open_driver_idx(i);
            if (h) {
                char tmp[V2U_SN_BUFSIZ];
                if (v2u_getsn(h, tmp, sizeof(tmp)) &&
                    strncmp(tmp, sn, sn_len) == 0) {

                    /* Serial number matched */
                    if (gid && gid[0]) {

                        /* Check if this is one of the several grabbers
                         * on a single board */
                        V2UPropertyValue v;
                        if (v2u_get_property(h, V2UKey_DeviceCaps, &v)) {
                            if (v.int32 & V2U_CAPS_MULTIGRAB) {

                                /* This board has several grabbers on it.
                                 * The driver must respond to the request
                                 * for V2UKey_GrabberId property */
                                if (v2u_get_property(h, V2UKey_GrabberId, &v)&&
                                    strncmp(v.str, gid, sizeof(v.str)) == 0) {
                                    /* Grabber ID matched */
                                    return h;
                                }
                            } else {
                                /* One grabber per board, ignore grabber id */
                                return h;
                            }
                        } else {
                            /* The driver didn't respond to V2UKey_DeviceCaps
                             * property, meaning that it's a really old driver.
                             * Old drivers didn't support multiple grabbers per
                             * board */
                            return h;
                        }
                    } else {
                        /* No grabber if was specified, any grabber will do */
                        return h;
                    }
                }
                v2u_close_driver(h);
 ...
 */
V2U_GrabFrame2* v2u_grab_frame2(V2U_HANDLE v2u, const V2URect* crop, int format)
{
    int bpp = V2UPALETTE_2_BPP(format);
    if (bpp && crop) {
        ...
        if (f) {
            ...
            f->pixbuf = malloc(f->pixbuflen);
            if (f->pixbuf) {
                if (v2u_ioctl(v2u, IOCTL_VGA2USB_GRABFRAME2, f, sizeof(*f))) {
                    return f;
                }
                free(f->pixbuf);
            }
